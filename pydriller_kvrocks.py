...
# Use local repository path instead of remote URL
repo_path = "../repos/kvrocks"  # Make sure this is the correct path to your cloned repo

# Define the problematic commit hash to skip
problem_commit = "28f50e0fed19872e0fd50dd23ce2ee8cd759338e"
...
with open(csv_filename, mode="w", newline="", encoding="utf-8") as csv_file:
    ...
    # Traverse the repository commits using the local repo path
    for commit in Repository(repo_path, only_no_merge=True).traverse_commits():
        
        # Skip the problematic commit
        if commit.hash == problem_commit:
            print(f"Skipping problematic commit: {commit.hash}")
            continue
        ...
        try:
            for file in commit.modified_files:
                ...
                if file.filename and (file.filename.endswith(('.o', '.obj', '.d', '.exe', '.bin', '.dll', '.so', '.class', '.jar')) or file.source_code is None):
                    continue
                
                # Check if the file is a README.md file
                if file.filename and file.filename.lower() == "readme.md":
                    latest_readme = file.source_code if file.source_code else "No content"
                
                # Check if the file is a LICENSE file
                if file.filename and (file.filename.lower() == "license" or file.filename.lower().startswith("license")):
                    latest_license = file.source_code if file.source_code else "No content"
                
                # Check if the file is a CONTRIBUTING.md file
                if file.filename and file.filename.lower() == "contributing.md":
